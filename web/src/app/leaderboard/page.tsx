...
export default function LeaderboardPage() {
  ...
  const getRankStyle = (rank: number) => {
    if (rank === 1) return "bg-gradient-to-r from-yellow-900/20 to-yellow-800/20 border-yellow-600";
    if (rank === 2) return "bg-gradient-to-r from-gray-700/20 to-gray-600/20 border-gray-500";
    if (rank === 3) return "bg-gradient-to-r from-orange-900/20 to-orange-800/20 border-orange-600";
            ...
            {loading ? (
              ...
              currentLeaderboard.map((player) => (
                <div
                  ...
                  className={`px-4 py-3 hover:bg-gray-800 transition-colors duration-150 border-l-4 cursor-pointer ${getRankStyle(player.rank)}`}
                >
                  <div className="grid grid-cols-12 gap-4 items-center">
                    {/* Rank */}
